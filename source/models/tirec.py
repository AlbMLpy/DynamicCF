...
class TIRec(TDRec):
    ...
    def _updater(self, da, mode: str):
        ...
        if mode == 'new_users' or mode == 'new_items':
            mode_v = 0 if mode == 'new_users' else 1
            self.u, self.v, self.w, self.core = update_tucker_new_vectors(
                (self.u, self.v, self.w), self.core,
                da, mode=mode_v, n_power_iter=self.n_power_iter,
                oversampling_factor=self.oversampling_factor,
                seed=self.seed,
                explicit_calc=self.explicit_calc,
            )
        elif mode == 'old':
            att_mtx = (
                self.attention_mtx 
    ...
    def update(self, data: pd.DataFrame, sort_bool: bool = True) -> None:
        ...
        # Update factors using 4-step process:
